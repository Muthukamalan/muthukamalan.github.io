...
excerpt: "Microservices are the architectural equivalent of dividing a single, manageable problem into twenty smaller problems that communicate via protocols"
---


![alt text](/../assets/project-kserve/image.png)

In this Space, I yap about three project:
- [votingapp](https://github.com/Muthukamalan/VotingApp)
- [wordwizards](https://github.com/Muthukamalan/WordWizards)
- [gcp-demo](https://github.com/googlecloudplatform/microservices-demo)
- [ML On Orchestr](https://github.com/Muthukamalan/)




## Voting App: Docker Compose -> K8s

Scope: Moving away from Docker compose and finally seeing K8s
{: .notice--success}

*A detailed look at [VotingApp](https://github.com/Muthukamalan/VotingApp) — a multi-language, multi-container distributed system built to make microservices concepts concrete, complete with observability and all the complexity budget goes toward the plumbing — queues, caches, persistence, exporters, tracing, dashboards — rather than the business logic.*



Most microservices tutorials show you one service talking to one database and call it a day. That's not really where the interesting problems live. The interesting problems show up when you have multiple languages, a message broker in between, a cache doing double duty as a task backend, and metrics/tracing wired through the whole thing — because that's what a real distributed system actually looks like. **VotingApp** is built around exactly that premise.
...
##  Word Wizards: K8s Manifests | Helm | Kustomize

Scope: Running as helm, kustomize & Istio cluster
{: .notice--success}

## GCP Microservices demo: Great Example
In the Era of AI, rewriting the entire app over the single PR is a `flex` 

Scope: Just running demo as expected, no new-innnovation
{: .notice--success}



## MLOps on the Kubernetes: Project

Scope: MLOps on the Open-source Orchestrator
{: .notice--success}
